angular.module('chefExpressApp.ingredientes')
	...
	.controller('ingredientesMainCtrl', function ($scope, ngTableParams, ingredientesAPI) {
		...
		$scope.sort = function (inputField) {
			var newOrder = null;
			if($scope.tablaIngredientes.sorting().order === 'asc') {
				var newOrder = 'desc';
			} else {
				var newOrder = 'asc';
			}
			$scope.tablaIngredientes.sorting()['field'] = inputField;
			$scope.tablaIngredientes.sorting()['order'] = newOrder;
